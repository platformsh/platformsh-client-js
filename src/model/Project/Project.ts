...
import { ProjectGetParams, AccountProjectType, ProjectType } from "./types";
...
export default class Project extends autoImplementWithResources()<Omit<ProjectType & AccountProjectType, "status">>() {
  ...
  addUser(user: string, role: string, byUuid = false) {
    ...
    if(accessLink) {
      const projectAccess = new ProjectAccess(body, accessLink);
