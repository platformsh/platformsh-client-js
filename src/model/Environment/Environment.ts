...
import { EnvironmentGetParams, EnvironmentQueryParams, EnvironmentType } from "./types";
...
export default class Environment extends autoImplementWithResources()<EnvironmentType>() {
  id: string;
  ...
  async getHeadCommit() {
    if(this.head_commit) {
      return await Commit.get(this.project, this.head_commit);
    }
  }
}
